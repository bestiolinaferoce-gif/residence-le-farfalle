...
import { Coffee, Wifi, Sparkles } from "lucide-react";
...
export default function ServiziHero() {
  return (
    <section
      ...
      aria-labelledby="servizi-hero-heading"
    >
      <div
        className="absolute inset-0 bg-[length:300%_300%] animate-territorio-gradient"
        style={{
          backgroundImage:
            "linear-gradient(135deg, #0369a1 0%, #0284c7 25%, #0d9488 40%, #14b8a6 60%, #2dd4bf 80%, #5eead4 100%)",
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div
          className="absolute -bottom-16 -right-16 w-80 h-80 rounded-full bg-cyan-300/20 blur-3xl"
          animate={{ x: [0, -20, 0], y: [0, 15, 0], scale: [1, 1.1, 1] }}
          transition={{ duration: 7, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute top-1/3 -left-12 w-56 h-56 rounded-full bg-teal-400/15 blur-2xl"
      ...
      <div className="absolute top-12 left-8 md:left-16 flex gap-4 opacity-70">
        <motion.div
          ...
          transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
        >
          <Coffee className="h-10 w-10 text-white" aria-hidden />
        </motion.div>
        <motion.div
          animate={{ y: [0, 6, 0], rotate: [0, -2, 0] }}
          transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
        >
          <Wifi className="h-10 w-10 text-cyan-100" aria-hidden />
        </motion.div>
        <motion.div
          animate={{ y: [0, -5, 0] }}
          transition={{ duration: 4.5, repeat: Infinity, ease: "easeInOut" }}
        >
          <Sparkles className="h-10 w-10 text-teal-100" aria-hidden />
        </motion.div>
      </div>
